...
from scorecomp import OVERALLONLY_CONFIG_NUMPY
...
@pytest.mark.parametrize(
    "args,expected_score_files",
    [
        ([], ["prediction.score"]),
        (["--numpy"], ["prediction.score"]),
        (["--numpy", "--detailed"], ["prediction.score"]),
        (["--numpy", "--wiedemann"], ["prediction.score", "prediction.score2"]),
        (["--numpy", "--detailed", "--wiedemann"], ["prediction.score", "prediction.score2"]),
    ],
)
def test_scorecomp_scoring_full_mask(caplog, args, expected_score_files):
    # speed up...
    scorecomp.EXPECTED_SHAPE = (10, 5, 11, 23, 8)
    ...
    ground_truth = np.full(shape=scorecomp.EXPECTED_SHAPE, fill_value=1, dtype=np.uint8)
    prediction = np.full(shape=scorecomp.EXPECTED_SHAPE, fill_value=3, dtype=np.uint8)
    full_mask = np.full(shape=scorecomp.EXPECTED_SHAPE, fill_value=1, dtype=np.uint8)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(log_file, "r") as f:
            ...
            logging.info(content)
            assert "completed ok" in content
        for fn in expected_score_files:
            score_file = os.path.join(temp_dir, fn)
            assert os.path.exists(score_file)
            with open(score_file, "r") as f:
                content = f.read()
...
def test_scorecomp_scoring_static_mask(caplog, args, expected_score_files):
    ...
    ground_truth = np.full(shape=scorecomp.EXPECTED_SHAPE, fill_value=1, dtype=np.uint8)
    prediction = np.full(shape=scorecomp.EXPECTED_SHAPE, fill_value=3, dtype=np.uint8)
    empty_static_mask = np.zeros(shape=(9, *(scorecomp.EXPECTED_SHAPE[-3:-1])), dtype=np.uint8)
...
def test_same_as_metrics_mse_implementation():
    ...
    #  TODO: torch and numpy implementation differ: (12566.040276092233, 12569.631067961163)!
    # mse_metrics = mse(actual=actual, expected=expected, mask=mask, use_np=True, mask_norm=True) #noqa
    # _mse = compute_mse_torch(actual=actual, expected=expected, mask=mask)#noqa
    # mse_scorecomp = _mse["mse_masked"]#noqa
    # assert np.isclose(mse_scorecomp, mse_metrics)#noqa

    mse_metrics = mse(actual=actual, expected=expected, mask=mask, use_np=True, mask_norm=True)
    _mse = compute_mse(actual=actual, expected=expected, full_mask=mask, config=OVERALLONLY_CONFIG_NUMPY)
...
def test_mse_wiedemann():
    ...
    score_dict = compute_mse(actual=actual, expected=expected, config=OVERALLONLY_CONFIG_NUMPY, wiedemann=True)
